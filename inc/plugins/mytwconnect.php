...
function mytwconnect_global()
{
	...
	$templatelist[] = 'mytwconnect_login_button';
	...
	if (strpos($pageURL, 'mytwconnect.php') !== false) {
		return true;
	}
...
function mytwconnect_usercp()
{
	...
	if (in_array($mybb->input['action'], ['mytwconnect_link', 'do_mytwconnect_link']) or $_SESSION['twlogin'] or ($mybb->input['action'] == 'mytwconnect' and $mybb->request_method == 'post')) {
	...
	// Authenticate
	if ($mybb->input['action'] == 'mytwconnect_link') {

		$TwitterConnect->set_fallback('usercp.php?action=do_mytwconnect_link');
		$TwitterConnect->authenticate();

	}

	// Link account to his Twitter's one
	if ($mybb->input['action'] == 'do_mytwconnect_link') {
	...
	if ($mybb->input['action'] == 'mytwconnect') {
		...
		if ($mybb->user['mytw_uid']) {
			...
			$text = $lang->mytwconnect_settings_what_to_sync;
